...
bluetooth () {
    ...
    case "$COMMAND" in
     "d") printf "^c$black^ ^b$blue^  ^d^%s" " ^c$blue^$(bluetoothctl info | grep Name: | sed 's/^[[:space:]]*//g' | sed -e "s/ //g" | sed 's/Name://g')" ;;
      *) 
        printf "" ;;
    esac
}
...
dwm_keyboard () {
  ...
  printf "^c$black^ ^b$green^ $LAYOUT ^b$black^"
}

wlan() {
	case "$(cat /sys/class/net/wl*/operstate 2>/dev/null)" in
	up) printf "^c$black^ ^b$blue^ 󰤨  ^d^%s" " ^c$blue^$(iwctl station wlp4s0 show | grep "Connected network" | awk '{print $NF}')" ;;
